...
const BASE_STATS = [
  ...
  },
  {
    ...
    label: "Total Revenue",
    base: 245680,
    icon: <IndianRupee size={22} strokeWidth={2.2} />,
    color: "green",
    isCurrency: true,
  },
  {
    ...
    base: 24,
    icon: <Luggage size={22} strokeWidth={2.2} />,
    color: "purple",
  },
];
...
const useStatsForDate = (dateKey) => {
  return useMemo(() => {
    ...
    return BASE_STATS.map((stat) => {
      const variance = 0.75 + rand() * 0.5;

      const value = Math.max(
        0,
        Math.round(stat.base * variance)
      );

      const change =
        Math.round((rand() * 30 - 6) * 10) / 10;
  ...
  return (
    ...
    >
      ...
      <div className="dashboard-home-stat-body">
        <span className="dashboard-home-stat-label">
          {stat.label}
        </span>

        <span className="dashboard-home-stat-value">
          {displayValue}
        </span>

        <span
          className={`dashboard-home-stat-trend ${
        ...
        >
          {isPositive ? (
            <ArrowUpRight
              size={14}
              className="dashboard-home-stat-trend-icon"
            />
          ) : (
            <ArrowDownRight
              size={14}
...
const DatePickerDropdown = ({
  ...
  return (
    <div
      className="dashboard-home-calendar-popover"
      ref={popoverRef}
    >
      <div className="dashboard-home-calendar-header">
        ...
        <span className="dashboard-home-calendar-month-label">
          {monthLabel}
        </span>
        ...
        {cells.map((day, i) => {
          if (!day) {
            return (
              <span
                key={`blank-${i}`}
                className="dashboard-home-calendar-day-blank"
              />
            );
          }
          ...
          return (
            <button
              type="button"
              key={day}
              className={`dashboard-home-calendar-day ${
                isToday ? "is-today" : ""
              } ${
      ...
      <div className="dashboard-home-calendar-footer">
        <button
          type="button"
          className="dashboard-home-calendar-today-btn"
          onClick={handleToday}
        >
          Today
        </button>
        ...
        <div className="dashboard-home-header">
          ...
          <div className="dashboard-home-header-text">
            <h1 className="dashboard-home-title">
              ...
              <span className="dashboard-home-wave">
                👋
              </span>
            </h1>
          ...
          <div className="dashboard-home-date-wrapper">
            ...
            <button
              type="button"
              className={`dashboard-home-date-btn ${
                isCalendarOpen
                  ? "is-active"
                  : ""
              }`}
            ...
            >
              <Calendar
                size={16}
                className="dashboard-home-date-icon"
              />
