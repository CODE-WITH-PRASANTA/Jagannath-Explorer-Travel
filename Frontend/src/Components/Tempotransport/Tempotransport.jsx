  ...
  return (
    <main className="tempotransport-page">
      ...
      <section className="tempotransport-header">
        <span className="tempotransport-subtitle">Rates &amp; Charges</span>
        <h1 className="tempotransport-title">Transport Tariff</h1>
      ...
      <section className="tempotransport-tariff-section">
        <div className="tempotransport-tariff-wrapper">
            ...
            <table className="tempotransport-table">
              <thead>
                <tr>
                  {tableHeaders.map((header) => (
                    <th key={header.key} className={`tempotransport-th tempotransport-th-${header.key}`}>
                      {header.label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {TRANSPORT_TARIFFS.map((item, index) => (
                  <tr key={`${item.vehicle}-${index}`} className={item.featured ? "tempotransport-featured-row" : ""}>
                    <td className="tempotransport-vehicle-cell">
                      <span className="tempotransport-vehicle-name">{item.vehicle}</span>
                      {item.featured && <span className="tempotransport-popular-badge">Popular</span>}
                    </td>
                    <td>{item.tenHrs}</td>
                    <td>{item.eightHrs}</td>
                    <td className={item.fourHrs === "—" ? "tempotransport-na" : ""}>{item.fourHrs}</td>
                    <td>{item.extraHrs}</td>
                    <td>{item.extraKm}</td>
                  </tr>
                ))}
          ...
          <div className="tempotransport-mobile-list">
            {TRANSPORT_TARIFFS.map((item, index) => (
              <article key={`mobile-${item.vehicle}-${index}`} className={`tempotransport-mobile-card ${item.featured ? "tempotransport-mobile-card-featured" : ""}`}>
                <div className="tempotransport-mobile-card-head">
                  <div className="tempotransport-mobile-vehicle">
                    <span className="tempotransport-mobile-vehicle-label">Vehicle A/C</span>
                    <h3>{item.vehicle}</h3>
                  </div>
                  {item.featured && <span className="tempotransport-mobile-badge">Popular</span>}
                </div>
